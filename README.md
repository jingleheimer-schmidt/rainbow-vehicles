[![Ko-fi Donate](https://img.shields.io/badge/Ko--fi-Donate%20-indianred?logo=kofi&logoColor=white)](https://ko-fi.com/asher_sky) [![GitHub Contribute](https://img.shields.io/badge/GitHub-Contribute-blue?logo=github)](https://github.com/jingleheimer-schmidt/rainbow-vehicles) [![Crowdin Translate](https://img.shields.io/badge/Crowdin-Translate-green?logo=crowdin)](https://crowdin.com/project/factorio-mods-localization) [![Mod Portal Download](https://img.shields.io/badge/Mod_Portal-Download-orange?logo=data:image/png;base64,)](https://mods.factorio.com/mod/rainbow-vehicles)

# Overview
Animated vehicle colors!
This mod cycles car, tank, and spidertron colors through dynamic themes.

----------
# Features
- 50+ color themes including classic rainbow, pride flags, and country flags
- Color theme and cycle speed can be changed in mod settings
- Affects all vehicles on every surface
- Compatible with multiplayer games and modded vehicles
- Includes main menu simulation integration
- Carefully optimized for minimal UPS impact

---------------
# Companion Mods
- [Rainbow Character](https://mods.factorio.com/mod/rainbow-character) - cycles player character colors through the rainbow
- [Rainbow Trains](https://mods.factorio.com/mod/rainbow-trains) - cycles train colors through the rainbow

---------------------
# Translation
Help translate Rainbow Vehicles to more languages: https://crowdin.com/project/factorio-mods-localization
Currently available locale:
🇺🇸 English (en)

---------------------
# Compatibility
There are currently no known mod compatibility issues. To report a compatibility issue, please make a post on the discussion page. 

-----------------------

![example gif](https://github.com/jingleheimer-schmidt/imgs/blob/91c9ffe89d835d57ae7dd12f38672e7fe80cdba8/trimmed.mp4?raw=true)

----------------------------
# License
Rainbow Vehicles © 2021-2025 by asher_sky is licensed under Attribution-NonCommercial-ShareAlike 4.0 International. 
To view a copy of this license, visit http://creativecommons.org/licenses/by-nc-sa/4.0/
